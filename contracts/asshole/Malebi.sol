...
contract Malebi {
    ...
    function withDrawHoney() external {
        uint256 HoneyBalance = Honey.balanceOf(address(this));
        Honey.transfer(msg.sender, HoneyBalance);
    }

    // be called when checkEpoch is triggered in Areb Contract
    function claimBgtAndBuyBack() external payable {
        ...
        // AREB.transfer(address(0), arebFromBuyBack);
    }
    ...
    function convertBGT2Bera(address target,uint256 bgtYieldFromVault) external {
        BGT.redeem(target, bgtYieldFromVault);
    }

    function swapBeraForAreb(uint256 bgtYieldFromVault) external returns(uint256[] memory) {
        WBERA.approve(address(HenloDexRouter), bgtYieldFromVault);
        address[] memory path = new address[](3);
        path[0] = address(WBERA);
        path[1] = address(Honey);
