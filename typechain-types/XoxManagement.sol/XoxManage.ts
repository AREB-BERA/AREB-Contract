/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import type {
  BaseContract,
  BigNumber,
  BytesLike,
  CallOverrides,
  PopulatedTransaction,
  Signer,
  utils,
} from "ethers";
import type { FunctionFragment, Result } from "@ethersproject/abi";
import type { Listener, Provider } from "@ethersproject/providers";
import type {
  TypedEventFilter,
  TypedEvent,
  TypedListener,
  OnEvent,
} from "../common";

export interface XoxManageInterface extends utils.Interface {
  functions: {
    "henduocanshu()": FunctionFragment;
    "sxr()": FunctionFragment;
    "upgrade()": FunctionFragment;
  };

  getFunction(
    nameOrSignatureOrTopic: "henduocanshu" | "sxr" | "upgrade"
  ): FunctionFragment;

  encodeFunctionData(
    functionFragment: "henduocanshu",
    values?: undefined
  ): string;
  encodeFunctionData(functionFragment: "sxr", values?: undefined): string;
  encodeFunctionData(functionFragment: "upgrade", values?: undefined): string;

  decodeFunctionResult(
    functionFragment: "henduocanshu",
    data: BytesLike
  ): Result;
  decodeFunctionResult(functionFragment: "sxr", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "upgrade", data: BytesLike): Result;

  events: {};
}

export interface XoxManage extends BaseContract {
  connect(signerOrProvider: Signer | Provider | string): this;
  attach(addressOrName: string): this;
  deployed(): Promise<this>;

  interface: XoxManageInterface;

  queryFilter<TEvent extends TypedEvent>(
    event: TypedEventFilter<TEvent>,
    fromBlockOrBlockhash?: string | number | undefined,
    toBlock?: string | number | undefined
  ): Promise<Array<TEvent>>;

  listeners<TEvent extends TypedEvent>(
    eventFilter?: TypedEventFilter<TEvent>
  ): Array<TypedListener<TEvent>>;
  listeners(eventName?: string): Array<Listener>;
  removeAllListeners<TEvent extends TypedEvent>(
    eventFilter: TypedEventFilter<TEvent>
  ): this;
  removeAllListeners(eventName?: string): this;
  off: OnEvent<this>;
  on: OnEvent<this>;
  once: OnEvent<this>;
  removeListener: OnEvent<this>;

  functions: {
    henduocanshu(overrides?: CallOverrides): Promise<[BigNumber]>;

    sxr(overrides?: CallOverrides): Promise<[string]>;

    upgrade(overrides?: CallOverrides): Promise<[BigNumber]>;
  };

  henduocanshu(overrides?: CallOverrides): Promise<BigNumber>;

  sxr(overrides?: CallOverrides): Promise<string>;

  upgrade(overrides?: CallOverrides): Promise<BigNumber>;

  callStatic: {
    henduocanshu(overrides?: CallOverrides): Promise<BigNumber>;

    sxr(overrides?: CallOverrides): Promise<string>;

    upgrade(overrides?: CallOverrides): Promise<BigNumber>;
  };

  filters: {};

  estimateGas: {
    henduocanshu(overrides?: CallOverrides): Promise<BigNumber>;

    sxr(overrides?: CallOverrides): Promise<BigNumber>;

    upgrade(overrides?: CallOverrides): Promise<BigNumber>;
  };

  populateTransaction: {
    henduocanshu(overrides?: CallOverrides): Promise<PopulatedTransaction>;

    sxr(overrides?: CallOverrides): Promise<PopulatedTransaction>;

    upgrade(overrides?: CallOverrides): Promise<PopulatedTransaction>;
  };
}
